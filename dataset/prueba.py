...
for i in queries1:
    list_queries1['name'].append('queries1_' + str(c1))
...
queries2_aux = wikidata_prefixes + df2['query'].astype(str)
queries2 = queries2_aux.to_list()
list_queries2 = {'name' : [], 'query' : []}
for i in queries2:
    list_queries2['name'].append('queries2_' + str(c2))
    list_queries2['query'].append(i)
    c2 += 1
...
for file in os.listdir(path_sparql_queries):
    if file.endswith(".rq"):
        ...
        list_queries3['query'].append(sparql_file)
queries3 = pd.DataFrame(list_queries3, columns=['name','query'])
queries3.to_csv(path_db+'queries/queries3.csv', index=False)


list_queries4 = {'name' : [], 'query' : []}
for file in os.listdir(path_sparql_queries_wikidata):
    if file.endswith(".rq"):
        list_queries4['name'].append("queries4_" + file)
        sparql_file = open(path_sparql_queries_wikidata+file, 'r', encoding = 'latin-1').read()
        list_queries4['query'].append(wikidata_prefixes+'\n'+sparql_file)
queries4 = pd.DataFrame(list_queries4, columns=['name','query'])
queries4.to_csv(path_db+'queries/queries4.csv', index=False)
